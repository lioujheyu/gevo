...
void particleFilter(unsigned char * I, int IszX, int IszY, int Nfr, int * seed, int Nparticles) {
    ...
    std::ofstream farrayX, farrayY, fweights;
    farrayX.open("arrayX");
    farrayY.open("arrayY");
    fweights.open("weights");

    for (x = 0; x < Nparticles; x++) {
        farrayX << arrayX[x] << std::endl;
        farrayY << arrayY[x] << std::endl;
        fweights << weights[x] << std::endl;
    }
