...
void particleFilter(unsigned char * I, int IszX, int IszY, int Nfr, int * seed, int Nparticles) {
    ...
    err = cuModuleGetFunction(&find_index_kernel, module, "_Z17find_index_kernelPdS_S_S_S_S_S_i");
    ...
    for (k = 1; k < Nfr; k++) {
        err = cuLaunchKernel(likelihood_kernel, dimGrid.x, dimGrid.y, dimGrid.z,
				                   dimBlock.x, dimBlock.y, dimBlock.z,
				   	               0, 0, likelihood_args, 0);
        err = cuLaunchKernel(sum_kernel, dimGrid.x, dimGrid.y, dimGrid.z,
				                   dimBlock.x, dimBlock.y, dimBlock.z,
				   	               0, 0, sum_args, 0);
        err = cuLaunchKernel(normalize_weights_kernel, dimGrid.x, dimGrid.y, dimGrid.z,
				                   dimBlock.x, dimBlock.y, dimBlock.z,
				   	               0, 0, normalize_args, 0);
        err = cuLaunchKernel(find_index_kernel, dimGrid.x, dimGrid.y, dimGrid.z,
				                   dimBlock.x, dimBlock.y, dimBlock.z,
				   	               0, 0, find_index_args, 0);
    ...
    std::stringstream farrayX, farrayY, fweights;
    ...
    std::ofstream result;
    result.open("arrayX");
    result.write(farrayX.str().c_str(), farrayX.str().length());
    result.close();
    result.open("arrayY");
    result.write(farrayY.str().c_str(), farrayY.str().length());
    result.close();
    result.open("weights");
    result.write(fweights.str().c_str(), fweights.str().length());
    result.close();
