...
void runTest( int argc, char** argv)
{
	...
	for( int i = 1 ; i <= block_width ; i++){
		dimGrid.x = i;
		dimGrid.y = 1;
		void *args1[] = {&referrence_cuda, &matrix_cuda, &max_cols, &penalty, &i, &block_width};
		// needle_cuda_shared_1<<<dimGrid, dimBlock>>>(referrence_cuda, matrix_cuda
		//                                       ,max_cols, penalty, i, block_width);
		err = cuLaunchKernel(kernel1, dimGrid.x, dimGrid.y, 1,
				                   dimBlock.x, 1, 1,
				   	               0, 0, args1, 0);
	}
	...
	for( int i = block_width - 1  ; i >= 1 ; i--){
		dimGrid.x = i;
		dimGrid.y = 1;
		void *args2[] = {&referrence_cuda, &matrix_cuda, &max_cols, &penalty, &i, &block_width};
		// needle_cuda_shared_2<<<dimGrid, dimBlock>>>(referrence_cuda, matrix_cuda
		//                                       ,max_cols, penalty, i, block_width);
		err = cuLaunchKernel(kernel2, dimGrid.x, dimGrid.y, 1,
				                   dimBlock.x, 1, 1,
				   	               0, 0, args2, 0);
	}
    ...
    for(int i=0; i<size; i++) {
      tempstr << output_itemsets[i] << std::endl;
    }
	result.write(tempstr.str().c_str(), tempstr.str().length());
    result.close();
