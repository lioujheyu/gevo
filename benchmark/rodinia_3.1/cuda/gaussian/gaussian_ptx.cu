...
void fPrintAry(float *ary, int ary_size);
...
void ForwardSub()
{
	...
	dim3 threads(blockSize2d,blockSize2d);
	dim3 grid(gridSize2d,gridSize2d);
	...
	for (t=0; t<(Size-1); t++) {
		...
		err = cuLaunchKernel(kernel2, grid.x, grid.y, grid.z,
				       threads.x, threads.y, threads.z,
				   	   0, 0, args2, 0);
		cudaThreadSynchronize();
		checkCUDAError("Fan2");
...
{
	int i;
	FILE *fp = fopen("final_solution","w");
	for (i=0; i<ary_size; i++) {
		fprintf(fp, "%.2f ", ary[i]);
	}
	// printf("\n\n");
