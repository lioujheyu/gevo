...
int main(int argc, char *argv []){
  ...
  printf("WG size of kernel = %d \n", NUMBER_THREADS);
	...
	err = cuModuleGetGlobal(&cudaPtr, NULL, module, "d_common");
	err = cuMemcpyHtoD(cudaPtr, &common, sizeof(params_common));

	// cudaMemcpyToSymbol(d_unique, &unique, sizeof(params_unique)*ALL_POINTS);
	err = cuModuleGetGlobal(&cudaPtr, NULL, module, "d_unique");
	err = cuMemcpyHtoD(cudaPtr, &unique, sizeof(params_unique)*ALL_POINTS);

	// cudaMemcpyToSymbol(d_common_change, &common_change, sizeof(params_common_change));
	err = cuModuleGetGlobal(&cudaPtr, NULL, module, "d_common_change");
	...
	for(common_change.frame_no=0; common_change.frame_no<frames_processed; common_change.frame_no++){
		...
		err = cudaMemcpy(common_change.d_frame, frame, common.frame_mem, cudaMemcpyHostToDevice);
		// cudaMemcpyToSymbol(d_common_change, &common_change, sizeof(params_common_change));
		err = cuMemcpyHtoD(cudaPtr, &common_change, sizeof(params_common_change));
		cudaProfilerStart();

		// launch GPU kernel
		// kernel<<<blocks, threads>>>();
		err = cuLaunchKernel(kernel, blocks.x, blocks.y, blocks.z,
