...
void initialize_variables(int nelr, float* variables)
{
	...
	void* args[] = {&nelr, &variables};
	CUfunction kernel;
	checkCudaErrors(cuModuleGetFunction(&kernel, module, "_Z25cuda_initialize_variablesiPf"));
	// cuda_initialize_variables<<<Dg, Db>>>(nelr, variables);
	cuLaunchKernel(kernel, Dg.x, Dg.y, Dg.z,
                       Db.x, Db.y, Db.z,
                       0, 0, args, 0);
...
void compute_step_factor(int nelr, float* variables, float* areas, float* step_factors)
{
	...
	CUfunction kernel;
	void* args[] = {&nelr, &variables, &areas, &step_factors};
	checkCudaErrors(cuModuleGetFunction(&kernel, module, "_Z24cuda_compute_step_factoriPfS_S_"));
...
void compute_flux(int nelr, int* elements_surrounding_elements, float* normals, float* variables, float* fluxes)
{
	...
	void* args[] = {&nelr, &elements_surrounding_elements, &normals, &variables, &fluxes};
	CUfunction kernel;
	checkCudaErrors(cuModuleGetFunction(&kernel, module, "_Z17cuda_compute_fluxiPiPfS0_S0_"));
...
void time_step(int j, int nelr, float* old_variables, float* variables, float* step_factors, float* fluxes)
{
	...
	void* args[] = {&j, &nelr, &old_variables, &variables, &step_factors, &fluxes};
	CUfunction kernel;
	checkCudaErrors(cuModuleGetFunction(&kernel, module, "_Z14cuda_time_stepiiPfS_S_S_"));
...
int main(int argc, char** argv)
{
  printf("WG size of kernel:initialize = %d, WG size of kernel:compute_step_factor = %d, WG size of kernel:compute_flux = %d, WG size of kernel:time_step = %d\n", BLOCK_SIZE_1, BLOCK_SIZE_2, BLOCK_SIZE_3, BLOCK_SIZE_4);
	...
	// set far field conditions and load them into constant memory on the gpu
	{
		...
		checkCudaErrors(cuModuleGetGlobal(&ff_variable, NULL, module, "ff_variable"));
		checkCudaErrors(cuMemcpyHtoD(ff_variable, &h_ff_variable, NVAR*sizeof(float)));
		// checkCudaErrors( cudaMemcpyToSymbol(ff_flux_contribution_momentum_x, &h_ff_flux_contribution_momentum_x, sizeof(float3)) );
		CUdeviceptr ff_flux_contribution_momentum_x;
		checkCudaErrors(cuModuleGetGlobal(&ff_flux_contribution_momentum_x, NULL, module, "ff_flux_contribution_momentum_x"));
		checkCudaErrors(cuMemcpyHtoD(ff_flux_contribution_momentum_x, &h_ff_flux_contribution_momentum_x, sizeof(float3)));
		// checkCudaErrors( cudaMemcpyToSymbol(ff_flux_contribution_momentum_y, &h_ff_flux_contribution_momentum_y, sizeof(float3)) );
		CUdeviceptr ff_flux_contribution_momentum_y;
		checkCudaErrors(cuModuleGetGlobal(&ff_flux_contribution_momentum_y, NULL, module, "ff_flux_contribution_momentum_y"));
		checkCudaErrors(cuMemcpyHtoD(ff_flux_contribution_momentum_y, &h_ff_flux_contribution_momentum_y, sizeof(float3)));
		// checkCudaErrors( cudaMemcpyToSymbol(ff_flux_contribution_momentum_z, &h_ff_flux_contribution_momentum_z, sizeof(float3)) );
		CUdeviceptr ff_flux_contribution_momentum_z;
		checkCudaErrors(cuModuleGetGlobal(&ff_flux_contribution_momentum_z, NULL, module, "ff_flux_contribution_momentum_z"));
		checkCudaErrors(cuMemcpyHtoD(ff_flux_contribution_momentum_z, &h_ff_flux_contribution_momentum_z, sizeof(float3)));
		...
		CUdeviceptr ff_flux_contribution_density_energy;
		checkCudaErrors(cuModuleGetGlobal(&ff_flux_contribution_density_energy, NULL, module, "ff_flux_contribution_density_energy"));
		checkCudaErrors(cuMemcpyHtoD(ff_flux_contribution_density_energy, &h_ff_flux_contribution_density_energy, sizeof(float3)));
	}
	int nel;
	int nelr;
